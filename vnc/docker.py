...
def container_port_id(containerID):
    ...
    portList = []
    process = subprocess.Popen(['docker', 'container', 'port', containerID],stdout=subprocess.PIPE)
    while True:
        output = process.stdout.readline()
        if process.poll() == 0:
            break
...
def docker_images_f_dangling_q():
    ...
    noneList = []
    process = subprocess.Popen(['docker', 'images', '-f', 'dangling=true', '-q'],stdout=subprocess.PIPE)
    while True:
        output = process.stdout.readline()
        if process.poll() == 0:
            break
...
if __name__ == '__main__':
    ...
    parser.add_argument("--delete_all_none_images", help="delete all name is none images", action='store_true')
